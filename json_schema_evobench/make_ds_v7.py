...
class SchemaUtils:
    ...
    @staticmethod
    def generate_example(schema: Dict, existing_data: Optional[Dict] = None) -> Dict:
        """
        根据 schema 生成示例数据
        如果提供 existing_data，会尝试保留兼容字段的值
        """
        example = {}
        ...
        # 如果有现有数据，尝试保留兼容字段的值
        if existing_data:
            for prop, value in existing_data.items():
                # 只保留schema中仍然存在的字段
                if prop in properties:
                    prop_def = properties[prop]
                    current_type = prop_def.get("type", "string")
                    ...
                    if current_type == "array" and isinstance(value, list):
                        ...
                        if item_type == "object":
                            ...
                            # 基本类型数组直接保留
                            example[prop] = value
                    elif current_type == "object" and isinstance(value, dict):
                        # 对象类型需要递归处理
                        example[prop] = SchemaUtils.generate_example(prop_def, value)
                    elif current_type in ["string", "integer", "number", "boolean"]:
                        # 基本类型直接保留
                        example[prop] = value
        ...
        for prop, definition in properties.items():
            ...
            if "enum" in definition:
                ...
                # 确保嵌套对象有内容
                nested_example = SchemaUtils.generate_example(definition)
                if nested_example:  # 只有当有内容时才添加
                    value = nested_example
            elif typ == "array":
                items_def = definition.get("items", {"type": "string"})
                min_items = definition.get("minItems", 1)  # 确保至少有一个元素
                max_items = definition.get("maxItems", 5)
                ...
                if items_def.get("type") == "object":
                    ...
                    for _ in range(num_items):
                        if items_def.get("type") == "string":
                            value.append(fake.word())
                        elif items_def.get("type") == "integer":
                            min_val = items_def.get("minimum", 0)
                            max_val = items_def.get("maximum", 100)
                            value.append(fake.random_int(min=min_val, max=max_val))
                        elif items_def.get("type") == "number":
                            min_val = items_def.get("minimum", 0)
                            max_val = items_def.get("maximum", 100)
                            value.append(round(min_val + random.random() * (max_val - min_val), 2))
                        elif items_def.get("type") == "boolean":
                            value.append(fake.boolean())
                        else:
                            value.append(fake.word())
    ...
    @staticmethod
    def migrate_data(old_data: Dict, old_schema: Dict, new_schema: Dict) -> Dict:
        """
        将旧数据迁移到新schema
        保留兼容字段的值，移除不再存在的字段，添加新字段
        """
        new_data = {}
        ...
        for field, value in old_data.items():
            if field in new_props:
                ...
                # 处理嵌套对象
                if new_type == "object" and old_type == "object":
                    # 递归迁移嵌套对象
                    new_data[field] = SchemaUtils.migrate_data(
                        value,
                        old_props[field],
                        new_props[field]
                    )
                # 处理数组
                    ...
                    new_item_type = new_props[field].get("items", {}).get("type")
                    old_item_type = old_props[field].get("items", {}).get("type")

                    if isinstance(value, list):
                        if new_item_type == "object" and old_item_type == "object":
                            # 迁移对象数组
                            new_data[field] = [
                                SchemaUtils.migrate_data(
                                    item,
                                    old_props[field]["items"],
                                    new_props[field]["items"]
                                )
                                for item in value
                            ]
                        else:
        ...
        for field, definition in new_props.items():
            if field not in new_data:
                # 生成新字段的值
                new_data[field] = SchemaUtils.generate_example(
